...
//            if (!executed)
        ...
        void initialize(bool loadLibs, bool init_lua = true) {
            _deallocQueue = new detail::DeallocQueue();

			this_owns_luastate = init_lua;
			if (init_lua)
			{
    ...
    public:
	    ...
	    explicit State(lua_State* luaState) : _luaState(luaState) { initialize(false, false); }
        
        ~State() {
			if (this_owns_luastate)
			{
				lua_close(_luaState);
			}
